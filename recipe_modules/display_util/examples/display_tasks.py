...
def GenTests(api):
  yield (
      ...
      api.step_data("Trigger Tests", api.swarming.trigger(["task1", "task2"])) +
      api.step_data(
          "collect",
  ...
  yield (
      api.status_check.test("Test Failure", status="failure") +
      api.step_data("Trigger Tests", api.swarming.trigger(["task1", "task2"])) +
      api.step_data(
          "collect",
          api.swarming.collect([
  ...
  yield (api.status_check.test("Test Success", status="success") +
         api.step_data("Trigger Tests", api.swarming.trigger(["task1"])) +
         api.step_data(
             "collect",
             api.swarming.collect([
                 api.swarming.task_result(id="0", name="task1"),
