...
def RunAndroidScenarioTests(api):
  ...
  with api.context(cwd = scenario_app_tests,
    ...
    if result.exc_result.retcode != 0:
      # Check if there are any diff failures.
      # If there are any, upload them to the cloud bucket.
      diff_failures = api.file.listdir(
          'List diff failures',
          scenario_app_tests.join('build', 'reports', 'diff_failures'),
          test_data = ['diff.png'])
...
def GenTests(api):
  for upload_packages in (True, False):
    yield api.test(
        'Scenario app (no failure, upload_packages: %d)' % upload_packages,
    ...
    test = api.test(
        'Scenario app (failure, upload_packages: %d)' % upload_packages,
        api.buildbucket.ci_build(
            builder='Linux Engine',
            git_repo='https://chromium.googlesource.com/external/github.com/flutter/engine',
            project='flutter',
